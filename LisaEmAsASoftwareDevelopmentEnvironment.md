...
- Download the latest release's "AppImage" file, e.g. https://github.com/arcanebyte/lisaem/releases/download/2.0.0/LisaEm-2.0.0-2026.01.25-Linux_x86_64.AppImage into your ~/Downloads folder

- Make the file "executable": `chmod +x ~/Downloads/LisaEm-2.0.0-2026.01.25-Linux_x86_64.AppImage`

- Launch it: `~/Downloads/LisaEm-2.0.0-2026.01.25-Linux_x86_64.AppImage` . This is it. The LisaEm UI window should show up.  How it works: Linux will automatically "unzip" the AppImage file into a new folder under "/tmp", and launch the `lisaem` binary from there. Once you close LisaEm, your Linux OS will automatically clean up (delete) that temporary folder. The LisaEm preferences file is typically in `~/lisaem.conf`, so it will not get deleted, which is what we want. Now is the time to check out the "Help->About LisaEm" popup dialog, and forget about it.

- Remember where your AppImage file is :) , or figure out how to create a shortcut to it, onto your Linux desktop.

- A newer LisaEm release is available? No problems: delete the old AppImage file, and follow again the steps above to download and run the new one.
...
Now give it a try: start LisaEm, and choose the "Key-> Power Button" menu. The Lisa emulator should successfully start, and, after some start up tests, you should be presented with a boot menu. Good. Close LisaEm and proceed below.
...
- Other LisaEm preferences: Launch again LisaEm, go to File->Preferences, and choose "I/O ROM: 88" from the drop-down menu. Reason: in this specific I/O ROM, the hard drive is the "top most" in the Lisa boot menu; this is needed, because the "Los Compilation Base" disk image (above) was created with this setting; you will get a System Error 10738 during boot time, if you choose a different I/O rom. Apply to save.
...
- We are finally ready to run this for real: Launch LisaEm and choose the "Key-> Power Button" menu to start the emulation. After a brief self-test, you will be presented with the Lisa boot menu. Your mouse should work on the Lisa screen:
...
- Launch Lisa Workshop and hit "T" (TransferProgram); it will launch a "Transfer" UI window, with its own menus. Use the menus to choose the following connection settings: `connector=PortB, baudrate=<any value you desire>, parity=None, handshake=None, duplex=Full`. 

- On the host Linux PC, install  the command-line utility `picocom` e.g. `sudo apt install picocom` and launch it: `picocom /tmp/b`; it should successfully connec to this port (`/tmp/b`, which is a symlink for the actual pseudo tty slave port, e.g. `/dev/pts/9 `).
...
## Can I compile the Lisa Office System source code using LisaEm, the way Alex Anderson-McLeod did it at https://github.com/alexthecat123/LisaSourceCompilation ?
