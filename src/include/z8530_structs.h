...
// These assume low bit is 1st (little-endian).
typedef union
{
  struct
  {
...
{
  struct
  { // high byte of time constant baud generator
    uint8 tc8 : 1,
        tc9 : 1,
        tc10 : 1,
        tc11 : 1,
...
{
  struct
  {               // interrupt vector written to wr2a, wr2b is status
...
{
  struct
  { // on escc msb of frame count on top of fifo - 8530 image of rr3
    uint8 bc8 : 1,
        bc9 : 1,
        bc10 : 1,
        bc11 : 1,
...
typedef union
{
  uint8 r[18]; // access will be scc_r[port].r[reg] or scc_r[port].s.rr0
  rr_t s;
} scc_r_t;

typedef union
{
  uint8 w[18]; // access will be scc_w[port].w[reg] or scc_w[port].s.wr0
  wr_t s;
} scc_w_t;

// This is here so that it can be accessed from z8530.c and z8530-pty.cpp.  The actual definition is in z8530.c.
extern scc_r_t scc_r[2];
extern scc_w_t scc_w[2];
