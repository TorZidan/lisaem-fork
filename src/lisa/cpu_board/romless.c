...
void romless_setvia_and_flopram(int profileboot)
{
  ...
  { // profile --------------
    ...
    if (!via[2].ProFile)
      return;
    if (via[2].ProFile->DC42.fd < 3 || via[2].ProFile->DC42.fh == NULL)
      return;
...
int romless_boot(int profileboot)
{
  ...
  for (i = 0; i < 128; i++)
  {
    ...
    mmu_all[1][i].changed = 3;
    mmu_all[2][i].sor = 0;
    mmu_all[2][i].slr = 0xc00;
    mmu_all[2][i].changed = 3;
    mmu_all[3][i].sor = 0;
    mmu_all[3][i].slr = 0xc00;
    mmu_all[3][i].changed = 3;
    mmu_all[4][i].sor = 0;
    mmu_all[4][i].slr = 0xc00;
    mmu_all[4][i].changed = 3;
  }

  mmu_all[1][0].sor = 0x400;
  mmu_all[1][0].slr = 0x700;
  mmu_all[1][1].sor = 0x500;
  mmu_all[1][1].slr = 0x700;
  mmu_all[1][2].sor = 0x600;
  mmu_all[1][2].slr = 0x700;
  mmu_all[1][3].sor = 0x700;
  mmu_all[1][3].slr = 0x700;
  mmu_all[1][4].sor = 0x800;
  mmu_all[1][4].slr = 0x700;
  mmu_all[1][5].sor = 0x900;
  mmu_all[1][5].slr = 0x700;
  mmu_all[1][6].sor = 0xa00;
  mmu_all[1][6].slr = 0x700;
  mmu_all[1][7].sor = 0xb00;
  mmu_all[1][7].slr = 0x700;
  ...
  for (i = 0; i < 0x560; i++)
    storebyte(i, romless_loram[i]);

  lisaram[0x80240] = serialnum240[0] >> 8;
  lisaram[0x80241] = serialnum240[0] & 0x0f;
  lisaram[0x80242] = serialnum240[1] >> 8;
  lisaram[0x80243] = serialnum240[1] & 0x0f;
  lisaram[0x80244] = serialnum240[2] >> 8;
  lisaram[0x80245] = serialnum240[2] & 0x0f;
  lisaram[0x80246] = serialnum240[3] >> 8;
  lisaram[0x80247] = serialnum240[3] & 0x0f;
  lisaram[0x80248] = serialnum240[4] >> 8;
  lisaram[0x80249] = serialnum240[4] & 0x0f;
  lisaram[0x8024a] = serialnum240[5] >> 8;
  lisaram[0x8024b] = serialnum240[5] & 0x0f;
  lisaram[0x8024c] = serialnum240[6] >> 8;
  lisaram[0x8024d] = serialnum240[6] & 0x0f;
  lisaram[0x8024e] = serialnum240[7] >> 8;
  lisaram[0x8024f] = serialnum240[7] & 0x0f;

  lisaram[0x80250] = serialnum240[8] >> 8;
  lisaram[0x80251] = serialnum240[8] & 0x0f;
  lisaram[0x80252] = serialnum240[9] >> 8;
  lisaram[0x80253] = serialnum240[9] & 0x0f;
  lisaram[0x80254] = serialnum240[10] >> 8;
  lisaram[0x80255] = serialnum240[10] & 0x0f;
  lisaram[0x80256] = serialnum240[11] >> 8;
  lisaram[0x80257] = serialnum240[11] & 0x0f;
  lisaram[0x80258] = serialnum240[12] >> 8;
  lisaram[0x80259] = serialnum240[12] & 0x0f;
  lisaram[0x8025a] = serialnum240[13] >> 8;
  lisaram[0x8025b] = serialnum240[13] & 0x0f;
  lisaram[0x8025c] = serialnum240[14] >> 8;
  lisaram[0x8025d] = serialnum240[14] & 0x0f;
  lisaram[0x8025e] = serialnum240[15] >> 8;
  lisaram[0x8025f] = serialnum240[15] & 0x0f;
  ...
  for (i = 0; i < 32768; i++)
  {
    ...
    dirtyvidram[i] = 0xff;
    if (j > 20)
      lisaram[videolatchaddress + i] = (j & 1 ? 0x55 : 0xaa);
    else if (j == 19)
      lisaram[videolatchaddress + i] = 0xff;
    else
      lisaram[videolatchaddress + i] = 0x00;
    ...
    if (profileboot == 0)
    {
      ...
      // profile nametag/led
      if (j > (b - 28) && j < (b - 20) && ((k > (l + 4) && k < (l + 15) && k != (l + 6)) || k == l + 47))
        lisaram[videolatchaddress + i] = 0xff;

      // feet
      if (j > b && j < b + 8 && (k == l + 2 || k == l + 10 || k == r - 2))
        lisaram[videolatchaddress + i] = 0xff;
    }
    else
    {
      ...
      if (j > t + 16 && j < t + 70 && (k == r - 20 || k == r - 13))
        lisaram[videolatchaddress + i] = 0xff;
      if (j > t + 8 && j < t + 16 && (k > r - 19 && k < r - 14))
        lisaram[videolatchaddress + i] = 0xff;
      if (j > t + 70 && j < t + 78 && (k > r - 19 && k < r - 14))
        lisaram[videolatchaddress + i] = 0xff;

      if (j > t && j < t + 98 && (k == l + 11 || k == r - 11))
        lisaram[videolatchaddress + i] = 0xff;
      if (j > t + 90 && j < t + 98 && (k > l + 11 && k < r - 11))
        lisaram[videolatchaddress + i] = 0xff;

      if (j > b - 150 && j < b && (k == l + 8 || k == r - 7))
        lisaram[videolatchaddress + i] = 0xff;
      if (j > b - 158 && j < b - 150 && (k > l + 8 && k < r - 7))
        lisaram[videolatchaddress + i] = 0xff;
    }
  }
...
void romless_proread(void)
{
  ...
  if (P == NULL)
  {
    ALERT_LOG(0, "ProFile Pointer for via:%d is null", vianum);
    return;
  }
  if ((&P->DC42) == NULL)
  {
    ALERT_LOG(0, "ProFile DC42 for via:%d Pointer is null", vianum);
    return;
  }
  ...
  if (sectornumber < 0x00f00000)
    sectornumber = deinterleave5(sectornumber);
  ...
  for (i = 0; i < 20; i++)
    storebyte(reg68k_regs[9] + i, sectordata[i + 512]); // a1
  for (i = 0; i < 512; i++)
    storebyte(reg68k_regs[10] + i, sectordata[i]); // a2
...
void romless_twgread(void)
{
  ...
  {
    ALERT_LOG(0, "failed!");
    reg68k_regs[0] = FLOP_STAT_NOREAD;
    reg68k_sr.sr_struct.c = 1;
    return;
  }
  for (i = 0; i < 512; i++)
    storebyte(reg68k_regs[10] + i, ptr[i]);
  ...
  for (i = 0; i < 12; i++)
    storebyte(reg68k_regs[9] + i, ptr[i]);
...
int romless_entry(void)
{
  ...
  if ((reg68k_pc & 0x00ffffff) == rom_profile_read_entry)
  {
    ...
    pc24 = reg68k_pc = fetchlong(reg68k_regs[15]);
    reg68k_regs[15] += 4; /* Simulate RTS */
    reg68k_regs[0] = 0;
    reg68k_regs[1] = 0;
...
int romless_dualparallel(void)
{
  ...
  dualparallelrom[4 + 0x0a] = 0x42;
  dualparallelrom[4 + 0x0b] = 0x40; // CLR.W D0
  dualparallelrom[4 + 0x0c] = 0x4e;
  dualparallelrom[4 + 0x0d] = 0x75; // RTS

  words = 0x7fe;
  newchks = chksum_a_rom_range(dualparallelrom, 0, words);
  newchks = (~newchks) + 1;
  dualparallelrom[words] = (newchks >> 8);
  dualparallelrom[words + 1] = (newchks & 0xff);
  return 0;
}
