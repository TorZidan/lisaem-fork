...
//
#define DC42_RET_CODE(F, code, msg, ret) \
   {                                     \
      if (F)                             \
      {                                  \
         F->retval = code;               \
...
{
   uint32 carry = (data & 1) << 31;
   return (data >> 1) | carry;
}

#define DC42_CHECK_VALID_F(F)                                                                      \
   {                                                                                               \
      if (!F)                                                                                      \
         return -1;                                                                                \
      if (!F->RAM)                                                                                 \
      {                                                                                            \
         DC42_RET_CODE(F, -3, "Disk image closed or no memory allocated to it", return F->retval); \
      }                                                                                            \
      if (F->fd < 3 && !F->fh)                                                                     \
      {                                                                                            \
         DC42_RET_CODE(F, -5, "Invalid file descriptor", return F->retval);                        \
      }                                                                                            \
   }

#define DC42_CHECK_VALID_F_NR(F)                                                         \
   {                                                                                     \
      if (!F)                                                                            \
         return;                                                                         \
      if (!F->RAM)                                                                       \
         DC42_RET_CODE(F, -3, "Disk image closed or no memory allocated to it", return); \
      if (F->fd < 3 && !F->fh)                                                           \
         DC42_RET_CODE(F, -5, "Invalid file descriptor", return);                        \
   }

#define DC42_CHECK_VALID_F_NUL(F)                                                             \
   {                                                                                          \
      if (!F)                                                                                 \
         return NULL;                                                                         \
      if (!F->RAM)                                                                            \
         DC42_RET_CODE(F, -3, "Disk image closed or no memory allocated to it", return NULL); \
      if (F->fd < 3 && !F->fh)                                                                \
         DC42_RET_CODE(F, -5, "Invalid file descriptor", return NULL);                        \
   }

#define DC42_CHECK_VALID_F_ZERO(F)                                                         \
   {                                                                                       \
      if (!F)                                                                              \
         return 0;                                                                         \
      if (!F->RAM)                                                                         \
         DC42_RET_CODE(F, -3, "Disk image closed or no memory allocated to it", return 0); \
      if (F->fd < 3 && !F->fh)                                                             \
         DC42_RET_CODE(F, -5, "Invalid file descriptor", return 0);                        \
   }

#define DC42_CHECK_WRITEABLE(F)                                        \
   {                                                                   \
      if (F->readonly)                                                 \
         DC42_RET_CODE(F, -8, "Image is Read Only", return F->retval); \
   }
#define DC42_CHECK_WRITEABLE1(F)                                       \
   {                                                                   \
      if (F->readonly == 1)                                            \
         DC42_RET_CODE(F, -8, "Image is Read Only", return F->retval); \
   }
...
#else
   if (F->fd)
      _commit(F->fd); // fucking microsoft!
#endif
...
int dc42_close_image_by_handle(DC42ImageType *F)
{
   ...
   if (F->fd > 2)
      F->fd = -1; // close the image, but not the file.
   if (F->fh)
      F->fh = NULL;
...
uint8 *dc42_read_sector_tags(DC42ImageType *F, uint32 sectornumber)
{
   ...
   *F->returnmsg = 0;
   ...
   {
      DC42_RET_CODE(F, 999, "invalid sector #", return NULL);
   }
...
int dc42_write_sector_data(DC42ImageType *F, uint32 sectornumber, uint8 *data)
{
   ...
   *F->returnmsg = 0;
   ...
   if (sectornumber > F->numblocks)
   {
      DC42_RET_CODE(F, 999, "invalid sector #", return 999);
   }
...
int dc42_is_valid_image(char *filename)
{
   ...
   if (F->fd < 3)
      DC42_RET_CODE(F, -6, "Cannot open the file.", return F->retval);
   F->fh = NULL;
#else
   ...
   if (tempbuf[0] > 64)
      DC42_RET_CODE(F, 0, "This is not a Disk Copy 4.2 image or is corrupt (DC42 volume name size >64 bytes)", return F->retval);
   if (tempbuf[82] != 1 || tempbuf[83] != 0)
      DC42_RET_CODE(F, 0, "This is not a Disk Copy 4.2 image or is corrupt (version is not 0100)", return F->retval);
...
int dc42_open(DC42ImageType *F, char *filename, char *options)
{
   ...
   if (tempbuf[0] > 64 || tempbuf[82] != 1 || tempbuf[83] != 0)
   ...
   if (F->readonly == 0)
   {
      ...
      if (F->fd < 3)
         DC42_RET_CODE(F, -86, "Cannot re-open file for writing", return F->retval);
      F->fh = NULL;
#else
...
int dc42_open_by_handle(DC42ImageType *F, int fd, FILE *fh, long seekstart, char *options)
{
   ...
   if (F->fd < 3 && !F->fh)
      DC42_RET_CODE(F, -86, "Cannot open file", return F->retval);
   if (F->fd > 2 && !!F->fh)
      DC42_RET_CODE(F, -86, "Cannot double open file, pick either fd or fh and set the other to null", return F->retval);
...
#ifdef HAVE_MMAPEDIO
   if (F->mmappedio)
      F->RAM = mmap(0, F->size, PROT_READ | PROT_WRITE, MAP_PRIVATE, F->fd, 0); // read only/private
   else
#endif
...
int RLEExpandBlock(uint8 *in, uint8 *out, int16 size, int sector)
{
   ...
   while (o < DART_CHUNK && i < os)
   {
      ...
      else if (s < 0) // repeat same word if negative
      {
         ...
         c2 = in[i + 1];
         i += 2;
         size--; // read the word to repeat
         do
         {
            out[o] = c1;
            out[o + 1] = c2;
            o += 2;
            s--; // repeat it to output s times
...
int dc42_add_tags(char *filename, uint32 tagsize) // tagsize is in bytes.  for a 400K disk use 400*2*12 for tagsize
{
   ...
   {
      fseek(image, newfilesize - 1, SEEK_SET);
      fputc(0, image);
   }
...
int dart_is_valid_image(char *dartfilename)
{
   ...
   if (!dart)
      return 0; // DC42_RET_CODE(F,0,"Cannot open DART file for reading",return F->retval);
   ...
   dart_compression_type = fgetc(dart);
   if (dart_compression_type > 2)
      return 0;
   dart_disk_type = fgetc(dart);
   if ((dart_disk_type > 3 && dart_disk_type < 16) || (dart_disk_type > 18))
      return 0;
   ...
   for (i = 0; i < dart_blocks; i++) // read the DART chunk sizes
   {
      ...
      if (feof(dart))
         return 0;
      if (dart_blocksize != 0)
         totalsizes += DART_CHUNK_TAGS; // calc disk size without tags
   }
...
int dart_to_dc42(char *dartfilename, char *dc42filename)
{
   ...
   dart = fopen(dartfilename, "rb");
   if (!dart)
      return -86; // DC42_RET_CODE(F,-86,"Cannot open DART file for writing",return F->retval);
   ...
   if (dart_disk_type != 1 || dart_disk_type != 16)
   {
      snprintf(volname, 62, "DART Converted image");
      volname_ptr = volname;
   }
   ...
   for (sectornum = 0, i = 0; i < dart_blocks && dart_blocksizes[i]; i++)
   {
      ...
      if (feof(dart))
         return -99; // DC42_RET_CODE(F,-99,"Unexpected EOF while reading DART file",return F->retval);
      ...
      { // read the compressed block and decompress it to the out buffer
         ...
         if (dart_compression_type == 0)
            ignore = fread(dart_block_in, dart_blocksizes[i] * 2, 1, dart);
         else
            ignore = fread(dart_block_in, dart_blocksizes[i], 1, dart);
         ...
         if (dart_compression_type == 0)
            RLEExpandBlock(dart_block_in, dart_block_out, dart_blocksizes[i], sectornum);
#ifdef USE_LZHUF
         if (dart_compression_type == 1)
            LZHExpandBlock(dart_block_in, dart_block_out, dart_blocksizes[i], sectornum);
#else
         if (dart_compression_type == 1)
...
int dc42_is_valid_macbinii(char *infilename, char *creatortype)
{
   ...
   if (!buffer[0] && buffer[1] && buffer[1] < 64 && !buffer[74] && !buffer[82])
   {
      ...
      if (header2 & 127)
         header2 = (header2 | 127) + 1;
      buffer[1] += 2;
      for (i = 2; i < buffer[1]; i++)
         if (buffer[i] == ':')
            return 0;

      if (datasize > 0x800000 || resrsize > 0x800000)
         return 0;

      if (creatortype != NULL)
      {
         for (i = 65; i < 73; i++)
            creatortype[i - 65] = buffer[i];
         creatortype[8] = 0;
      }
...
int dc42_extract_macbinii(char *infilename)
{
   ...
   if (!buffer[0] && buffer[1] && buffer[1] < 64 && !buffer[74] && !buffer[82])
   {
      ...
      FILE *datafork = fopen(filename, "wb");
      if (!datafork)
      {
         fclose(infile);
         return -3;
...
int dc42_auto_open(DC42ImageType *F, char *infilename, char *options)
{
   ...
   if (dart_is_valid_image(filename))
   {
      ...
      if (!i)
         i = dc42_open(F, dc42filename, options);
      return i;
   }
...
int replacesec(DC42ImageType *F, int sector, int size, uint8 *s, uint8 *r)
{
   ...
   if (count && r != NULL)
      dc42_write_sector_data(F, sector, d);
...
#define putc(z, outfile)                           \
   {                                               \
      if (PutFilePosition <= EndOfPutFilePosition) \
         *PutFilePosition++ = z;                   \
   }
...
static void save_huff(void)
{
   if (!saved_lson)
   {
      saved_lson = malloc((N + 1) * sizeof(int16));
      memcpy(saved_lson, lson, (N + 1) * sizeof(int16));
   }
   if (!saved_rson)
   {
      saved_rson = malloc((N + 257) * sizeof(int16));
      memcpy(saved_rson, rson, (N + 257) * sizeof(int16));
   }
   if (!saved_dad)
   {
      saved_dad = malloc((N + 1) * sizeof(int16));
      memcpy(saved_dad, dad, (N + 1) * sizeof(int16));
   }
   if (!saved_freq)
   {
      saved_freq = malloc((T + 1) * sizeof(uint16));
      memcpy(saved_freq, freq, (T + 1) * sizeof(uint16));
   }
   if (!saved_son)
   {
      saved_son = malloc((T) * sizeof(int16));
      memcpy(saved_son, son, (T) * sizeof(int16));
   }
   if (!saved_prnt)
   {
      saved_prnt = malloc((T + N_CHAR) * sizeof(int16));
      memcpy(saved_prnt, prnt, (T + N_CHAR) * sizeof(int16));
   }
}
